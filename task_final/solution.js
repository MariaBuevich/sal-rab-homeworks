...
function sendRequest(name, phone, address, goods, sum) {
    ...
    for (let i = 0; i < countOfGoods; i += 1) {
        ...
        data.goods.push(objectGood);
    }

    let superAddress = 'ул. '+ address.street + ', дом '+ address.house + ', ' + address.entrance + ' подъезд, ' + address.floor + ' этаж, кв ' + address.flat;
    data.order.address = superAddress;
    data.order.sum =  sum;
    data.client = name + ' ' + phone;
    let doneData = JSON.stringify({data});
    return doneData;
}
